/** cordova-plugin-inapppurchases MIT © 2023 cozycode.ca edit **/
...
/**
 * Array.find() Polyfill https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/find
 */
...
/*utils.chunk = function (array, size) {
    ...
    if (typeof size !== 'number' || size < 1) throw new Error('Invalid size');
...
/*utils.max_product_ids = 19;
utils.chunkProductIds = function (productIds) {
...
inAppPurchases.getPurchases = function(){
    return nativeCall('billingGetPurchases').then( function(purchases){
        try {
            if (purchases.length == 0) return Promise.resolve([]);
